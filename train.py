...
def get_args():
    ...
    parser.add_argument("--dataset_dir", type=str, default='E:/dataset/HDR/data',
    ...
    # Training  ./checkpoints/val_latest_checkpoint.pth
...
def validation(args, model, device, val_loader, optimizer, epoch, criterion, cur_psnr):
    ...
    with open(os.path.join(args.logdir, 'psnr_scconv_drdb_hpb.json'), 'a') as f:
        f.write('epoch:' + str(epoch) + '\t')
        f.write('Validation set: Average Loss: {:.4f}'.format(val_loss.avg) + '\t')
        f.write('Validation set: Average PSNR: {:.4f}, mu_law: {:.4f}'.format(val_psnr.avg, val_mu_psnr.avg) + '\n')
